    ...
    <div class="container">
                            ...
                            <table style="width:100%; margin-bottom:0; background:#f9f9fa; border-radius:6px;">
                                ...
                                @foreach ($ordem->itens as $item)
                                    ...
                                    <tr>
                                        <td style="font-size:11px;">
                                            {{ $item->pecaServico->tipo == 1 ? 'Serviço' : 'Peça' ?? '' }}</td>
                                        <td style="font-size: 11px">
                                            @php
                                                $tipos = [
                                                    1 => 'Preventiva',
                                ...
                                <tr>
                                    <td colspan="7" style="text-align:right; font-size:13px; background:#f2f2f4;">
                                        <strong>Total Preventiva:</strong> R$
                                        {{ number_format($totalPreventiva, 2, ',', '.') }} &nbsp; | &nbsp;
                                        <strong>Total Corretiva:</strong> R$
                                        {{ number_format($totalCorretiva, 2, ',', '.') }} &nbsp; | &nbsp;
                                        <strong>Total Avaria:</strong> R$
                                        {{ number_format($totalAvaria, 2, ',', '.') }} &nbsp; | &nbsp;
                                        <strong>Total Multa:</strong> R$
                                        {{ number_format($totalMulta, 2, ',', '.') }} &nbsp; | &nbsp;
                                        <strong>Total Outros:</strong> R$
        ...
        <table style="margin-top:32px;">
            ...
            <tr>
                ...
                <td style="text-align:right; font-size:15px; background:#f2f2f4; padding:10px;">R$
                    {{ number_format($totalPreventivaGeral, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td style="font-size:15px; background:#f2f2f4; padding:10px;"><strong>Total Geral Corretiva:</strong>
                </td>
                <td style="text-align:right; font-size:15px; background:#f2f2f4; padding:10px;">R$
                    {{ number_format($totalCorretivaGeral, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td style="font-size:15px; background:#f2f2f4; padding:10px;"><strong>Total Geral Avaria:</strong></td>
                <td style="text-align:right; font-size:15px; background:#f2f2f4; padding:10px;">R$
                    {{ number_format($totalAvariaGeral, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td style="font-size:15px; background:#f2f2f4; padding:10px;"><strong>Total Geral Multa:</strong></td>
                <td style="text-align:right; font-size:15px; background:#f2f2f4; padding:10px;">R$
                    {{ number_format($totalMultaGeral, 2, ',', '.') }}</td>
            </tr>            
            <tr>
                <td style="font-size:15px; background:#f2f2f4; padding:10px;"><strong>Total Geral Outros:</strong></td>
                <td style="text-align:right; font-size:15px; background:#f2f2f4; padding:10px;">R$
                    {{ number_format($totalOutrosGeral, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td colspan="2" style="text-align: right; font-size: 16px; padding-top:16px;"><strong>Total de
                        Ordens:</strong>
                    <strong>{{ $ordemServicoRelatorio->count() }}</strong>
                </td>
            </tr>
        </table>
    </div>
